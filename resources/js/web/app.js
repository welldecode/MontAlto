...
function initSelect2() {
    $('.select2').each(function () {
        ...
        // pega valor real do Livewire
        let model = $this.data('model');
        let component = Livewire.find(document.querySelector('[wire\\:id]').getAttribute('wire:id'));
        let livewireValue = model && component ? component.get(model) : null;

        // recria select2
        $this.select2({ width: '100%' });

        // aplica valor
        if (livewireValue) $this.val(livewireValue).trigger('change.select2');

        // evento onchange -> atualiza Livewire
...
document.addEventListener("livewire:load", () => {
    console.log('Evento livewire:load disparado');
    setTimeout(() => {
        initSelect2();
    }, 50);
});
...
Livewire.on('refreshSelect2', () => {
    console.log('Refresh Select2 disparado');
    setTimeout(() => {
        initSelect2();
    }, 50);
});
